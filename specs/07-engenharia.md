...
3. Revisão do `applicationId` (ainda o padrão `com.dsdev.pdf.ds_pdf` gerado pelo `flutter create`) e do nome de exibição do app (`android:label` ainda é `"ds_pdf"`) antes da publicação — o `applicationId` não pode mudar depois do primeiro envio à Play Store. Assinatura de release já resolvida (ver Riscos acima).
